...
export class RelationshipModal extends Modal {
	...
	private addPersonInput(container: HTMLElement, options: PersonPickerOptions): FieldControl<HTMLInputElement> {
		...
		const listId = `people-atlas-person-suggestions-${++modalSequence}`;
		...
		const close = (): void => {
			...
			input.setAttribute("aria-expanded", "false");
			updateActive();
			if (this.closeActivePersonPicker === close) this.closeActivePersonPicker = undefined;
		};
		...
		const open = (): void => {
			this.activatePersonPicker(close);
